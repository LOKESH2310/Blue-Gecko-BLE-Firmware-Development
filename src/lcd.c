...
#define LCD_UPDATE_TIMER_PERIOD  32768 // 1 second in Bluetooth soft timer units
#define LCD_TIMER_HANDLE 0
...
void displayInit()
{
    ...
    Si7021_PowerOn(); //turn on the LCD
    ...
    // Edit #3: Set up a Bluetooth soft timer for 1Hz updates
    sl_status_t timer_response = sl_bt_system_set_lazy_soft_timer(
        LCD_UPDATE_TIMER_PERIOD, // Time period (ticks)
        0,  // Lazy: Set to 0 (triggers immediately after timeout)
        LCD_TIMER_HANDLE, // Handle to identify the timer
        0   // Single shot: 0 for periodic execution
    );
    if (timer_response != SL_STATUS_OK) {
        LOG_ERROR("Failed to set LCD update soft timer: 0x%x", timer_response);
    }
...
void displayUpdate()
{
  ...
  // Students: Create the function gpioSetDisplayExtcomin() that will set
	...
	//
	gpioSetDisplayExtcomin(display->last_extcomin_state_high); //set extcomin input to the lcd
